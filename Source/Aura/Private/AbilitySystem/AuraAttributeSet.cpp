...
void UAuraAttributeSet::SetEffectProperties(const FGameplayEffectModCallbackData& Data, FEffectProperties& Props) const
{
	...
	// Target = target of the effect (owner of AS)
...
void UAuraAttributeSet::PostAttributeChange(const FGameplayAttribute& Attribute, float OldValue, float NewValue)
{
	...
	else if (bTopOffMana && Attribute  == GetMaxManaAttribute())
	{
		SetMana(GetMaxMana());
		bTopOffMana = false;
	}
...
void UAuraAttributeSet::SendXPEvent(const FEffectProperties& Props)
{
	if (Props.TargetCharacter->Implements<UCombatInterface>())
	{
		...
		const int32 TargetXPReward = UAuraAbilitySystemLibrary::GetXPRewardForCharacterClassAndLevel(Props.TargetCharacter, TargetCharacterClass, TargetLevel);

		const FAuraGameplayTags& AuraGameplayTags = FAuraGameplayTags::Get();

		FGameplayEventData Payload;
		Payload.EventTag = AuraGameplayTags.Attributes_Meta_IncomingXP;
		Payload.EventMagnitude = TargetXPReward;
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		{
			if (Props.TargetCharacter->Implements<UCombatInterface>() && !ICombatInterface::Execute_GetInBeingShock(Props.TargetCharacter))
			{
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	if (MutableSpec)
	{
		...
		AuraContext->SetDamageType(MakeShareable<FGameplayTag>(new FGameplayTag(DamageType)));
	}
